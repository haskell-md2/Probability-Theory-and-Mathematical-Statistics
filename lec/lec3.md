...
$$P(S_n^{(1)} = k_1,\ \ldots S_n^{(m)} = k_m) = C_n^{k_1}\cdot C_{n-k_1}^{k_2}\cdot C_{n-k_1-k_2}^{k_3}\cdot \ldots \cdot C_{n-k_1-\ldots - k_{m-1}}^{k_m}\cdot  p_1^{k_1}\cdot p_2^{k_2}\cdot p_3^{k_3}\cdot \ldots \cdot p_m^{k_m}$$
...
\underbrace{\left(1-\frac{\lambda}{n}\right)^{n}}_{\rightarrow e^{-\lambda}}
$$
...
Условия применимости теоремы Пуассона на практике: 
$$n>>1,\ np=\lambda \approx 1$$

Также есть теорема для оценки погрешности, которая получается при использовании теоремы Пуассона: 

> Теорема (о погрешности)

$$\left| P(S_n =k, \forall k = \overline{0,n}) - \sum_{k=\overline{0,n}} \frac{(np)^k}{k!}e^{-np}\right|\leq np^2$$

Но есть и более удобная теорема. 
...
$$C_n^k = \frac{n!}{k!(n-k)!}=\frac{\sqrt{2\pi n} n^n e^{-n}}{\sqrt{2\pi k} k^k e^{-k}\cdot \sqrt{2\pi (n-k)} (n-k)^{(n-k)} e^{-(n-k)}} = \sqrt{\frac{n}{2\pi k (n-k)}} \left( \frac{n}{k}\right)^k  \left( \frac{n}{n - k}\right)^{n-k}$$
